...
import {showTables} from "@/components/Scheduler/SchedulerEvent/DDL";
import {JarStateType} from "@/pages/RegistrationCenter/Jar/model";
import {Scrollbars} from "react-custom-scrollbars";
import {RUN_MODE} from "@/components/Scheduler/conf";
...
const SchedulerSetting = (props: any) => {
      ...
      <Scrollbars style={{height: (toolHeight - 32)}}>
        ...
        >
          <Form.Item
            ...
            tooltip={l('pages.Scheduler.label.jobConfig.execmode.tip')}
          >
          ...
          {(current.task.type === RUN_MODE.YARN_SESSION || current.task.type === RUN_MODE.KUBERNETES_SESSION || current.task.type === RUN_MODE.STANDALONE) ? (
            <Row>
              <Col span={24}>
                <Form.Item label={l('pages.Scheduler.label.jobConfig.cluster')}
                           tooltip={l('pages.Scheduler.label.jobConfig.clusterConfig.tip1', '', {
                           ...
                           className={styles.form_item}>
                  {
                    currentSession.session ?
                      (currentSession.sessionConfig && currentSession.sessionConfig.clusterId ?
                        ...
                        style={{width: '100%'}}
                        placeholder={l('pages.Scheduler.label.jobConfig.cluster.tip')}
          ...
          {(current.task.type === RUN_MODE.YARN_PER_JOB || current.task.type === RUN_MODE.YARN_APPLICATION || current.task.type === RUN_MODE.KUBERNETES_APPLICATION) ? (
            <Row>
              <Col span={24}>
                <Form.Item label={l('pages.Scheduler.label.jobConfig.clusterConfig')}
                           tooltip={l('pages.Scheduler.label.jobConfig.clusterConfig.tip1', '', {
                             type: current.task.type
                           })}
                  ...
                  <Select
                    style={{width: '100%'}}
                    placeholder={l('pages.Scheduler.label.jobConfig.clusterConfig.tip2')}
                    optionLabelProp="label"
                  >
          ...
          <Form.Item label={l('pages.Scheduler.label.jobConfig.flinksql.env')}
                     tooltip={l('pages.Scheduler.label.jobConfig.flinksql.env.tip1')}
                     name="envId"
                     className={styles.form_item}>
            <Select
              style={{width: '100%'}}
              placeholder={l('pages.Scheduler.label.jobConfig.flinksql.env.tip2')}
              allowClear
              optionLabelProp="label"
          ...
          <Row>
            <Col span={12}>
              <Form.Item
                label={l('pages.Scheduler.label.jobConfig.parallelism')} className={styles.form_item}
                name="parallelism"
                tooltip={l('pages.Scheduler.label.jobConfig.parallelism.tip')}
              >
            ...
            <Col span={12}>
              <Form.Item
                label={l('pages.Scheduler.label.jobConfig.insert')} className={styles.form_item} name="statementSet"
                valuePropName="checked"
                tooltip={{
                  title: l('pages.Scheduler.label.jobConfig.insert.tip'),
          ...
          <Row>
            <Col span={12}>
              <Form.Item
                label={l('pages.Scheduler.label.jobConfig.fragment')} className={styles.form_item} name="fragment"
                valuePropName="checked"
                tooltip={{
                  title: l('pages.Scheduler.label.jobConfig.fragment.tip'),
                  icon: <InfoCircleOutlined/>
                }}
              >
                <Switch checkedChildren={l('button.enable')} unCheckedChildren={l('button.disable')}
            ...
            <Col span={12}>
              <Form.Item
                label={l('pages.Scheduler.label.jobConfig.batchmode')} className={styles.form_item} name="batchModel"
                valuePropName="checked"
                tooltip={{title: l('pages.Scheduler.label.jobConfig.batchmode.tip'), icon: <InfoCircleOutlined/>}}
              >
                <Switch checkedChildren={l('button.enable')} unCheckedChildren={l('button.disable')}
                />
              </Form.Item>
          ...
          <Form.Item
            label={l('pages.Scheduler.label.jobConfig.savePointStrategy')} className={styles.form_item}
            name="savePointStrategy"
            tooltip={l('pages.Scheduler.label.jobConfig.savePointStrategy.tip')}
          >
          ...
          {current.task.savePointStrategy === 3 ?
            (<Form.Item
              label={l('pages.Scheduler.label.jobConfig.savePointpath')} className={styles.form_item}
              name="savePointPath"
              tooltip={l('pages.Scheduler.label.jobConfig.savePointpath.tip1')}
            >
              <Input placeholder={l('pages.Scheduler.label.jobConfig.savePointpath.tip2')}/>
            </Form.Item>) : ''
          }
          <Row>
            <Col span={24}>
              <Form.Item label={l('pages.Scheduler.label.jobConfig.alertGroup')} name="alertGroupId"
                         className={styles.form_item}>
                <Select
                  style={{width: '100%'}}
                  placeholder={l('pages.Scheduler.label.jobConfig.alertGroup.tip')}
                  optionLabelProp="label"
                  defaultValue={0}
                >
                  {getGroupOptions()}
          ...
          <Form.Item
            label={l('pages.Scheduler.label.jobConfig.other')} className={styles.form_item}
            tooltip={{title: l('pages.Scheduler.label.jobConfig.other.tip'), icon: <InfoCircleOutlined/>}}
          >
                ...
                <>
                  {fields.map(({key, name, fieldKey, ...restField}) => (
                    <Space key={key} style={{display: 'flex'}} align="baseline">
                      ...
                      >
                        <Input placeholder={l('pages.Scheduler.label.jobConfig.addConfig.params')}/>
                      </Form.Item>
                      <Form.Item
                        {...restField}
                        name={[name, 'value']}
                        style={{marginBottom: '5px'}}
                      >
                        <Input placeholder={l('pages.Scheduler.label.jobConfig.addConfig.value')}/>
                      </Form.Item>
                  ...
                  <Form.Item>
                    <Button type="dashed" onClick={() => add()} block icon={<PlusOutlined/>}>
                      {l('pages.Scheduler.label.jobConfig.addConfig')}
                    </Button>
                  </Form.Item>
                </>
              )}
...
export default connect(({Scheduler, Jar, Alert}: { Scheduler: StateType, Jar: JarStateType, Alert: AlertStateType }) => ({
  sessionCluster: Scheduler.sessionCluster,
  clusterConfiguration: Scheduler.clusterConfiguration,
