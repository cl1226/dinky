...
import {convertToTreeData, getTreeNodeByKey, TreeDataNode} from "@/components/Scheduler/SchedulerTree/Function";
...
const SchedulerTree: React.FC<SchedulerTreeProps> = (props) => {
  ...
  const toOpen = (node: TreeDataNode | undefined) => {
    ...
    if (node?.isLeaf && node.taskId) {
      ...
      const result = getInfoById('/api/workflow/task', node.taskId);
      result.then(result => {
        let newTabs = tabs;
        let newPane: any = {
          title: node.name,
    ...
    <div className={style.tree_div}>
      ...
      <Scrollbars style={{height: (size.height - 84 - 72)}} ref={sref}>
          ...
          <UpdateCatalogueForm
            onSubmit={async (value) => {
              const success = await handleAddOrUpdate(
                isCreate ? '/api/workflow/catalogue' : '/api/workflow/catalogue/toRename', value);
              if (success) {
        ...
        {updateTaskModalVisible ? (
          <SimpleTaskForm
            onSubmit={async (value) => {
              const datas = await handleAddOrUpdateWithResult('/api/workflow/catalogue/createTask', value);
              if (datas) {
      ...
      <UploadModal visible={isUploadModalVisible} action={`/api/workflow/catalogue/upload/${uploadNodeId}`} handleOk={() => {
