...
import {explainSql} from "@/pages/Scheduler/service";
...
export type SchedulerExplainProps = {
  onCancel: (flag?: boolean) => void;
  modalVisible: boolean;
  data: Partial<ExplainItem>;
}
const SchedulerExplain = (props: any) => {

  const [explainData, setExplainData] = useState([]);
  const [result, setResult] = useState(<Text>{l('pages.Scheduler.explain.validate')}</Text>);
  const {
    onClose,
    modalVisible,
    current,
  ...
  useEffect(() => {
    ...
    setResult(<Text>{l('pages.Scheduler.explain.validate')}</Text>);
    setExplainData([]);
    ...
    result.then(res => {
      ...
      if (errorCount == 0) {
        ...
        setResult(<Text type="success">{l('pages.Scheduler.explain.validate.allright')}</Text>);
      } else {
        setExplainData(errorExplainData);
        setResult(<Text type="danger">{l('pages.Scheduler.explain.validate.error','',{errorCount: errorCount})}</Text>);
      }
    })
  }, [modalVisible]);
            ...
            subTitle: {
              render: (_, row) => {
                return (
                  <Space size={0}>
                    {row.parseTrue ?
                      (<Tag color="#44b549">{l('pages.Scheduler.explain.validate.grammar.right')}</Tag>) :
                      (<Tag color="#ff4d4f">{l('pages.Scheduler.explain.validate.grammar.error')}</Tag>)}
                    {row.explainTrue ?
                      (<Tag color="#108ee9">{l('pages.Scheduler.explain.validate.logic.right')}</Tag>) :
                      (<Tag color="#ff4d4f">{l('pages.Scheduler.explain.validate.logic.error')}</Tag>)}
                    {row.explainTime}
                  </Space>
                );
              },
  ...
  return (
    <Modal
      ...
      destroyOnClose
      centered
      title={l('pages.Scheduler.explain.validate.msg')}
