...
import {cancelJob, savepointJob, showFlinkJobs} from "../../SchedulerEvent/DDL";
...
const SchedulerProcess = (props: any) => {
    ...
    <Dropdown
      overlay={
        <Menu onClick={({key}) => savepoint(key, item)}>
          <Menu.Item key="trigger">{l('pages.Scheduler.label.process.trigger')}</Menu.Item>
          <Menu.Item key="stop">{l('pages.Scheduler.label.process.stop')}</Menu.Item>
          <Menu.Item key="cancel">{l('pages.Scheduler.label.process.cancel')}</Menu.Item>
        </Menu>
      }
    >
      <a>
        {l('pages.Scheduler.label.process.savepoint')}  <DownOutlined/>
      </a>
    </Dropdown>
  );
  ...
  const getColumns = () => {
    let columns: any = [{
      ...
      render: (_, record) => {
        ...
        >
          {l('pages.Scheduler.label.process.detail')}
        </a>];
        if (record.state == 'RUNNING' || record.state == 'RECONCILING' || record.state == 'SCHEDULED') {
          ...
          >
            {l('pages.Scheduler.label.process.stop')}
          </a>);
        }
  ...
  const onCancel = (jobId: string) => {
    Modal.confirm({
      title: l('pages.Scheduler.label.process.stopconfirm','',{jobid:jobId}),
      okText: l('pages.Scheduler.label.process.stop'),
      cancelText: l('button.cancel'),
      onOk: async () => {
        if (!clusterId) return;
        ...
        res.then((result) => {
          if (result.datas == true) {
            message.success(l('pages.Scheduler.label.process.stopsuccess'));
            onRefreshJobs();
          } else {
            message.error(l('pages.Scheduler.label.process.stopfailed'));
          }
        });
      }
    });
  ...
  return (
    <div style={{width: '100%'}}>
      <Space>
        <Select
          // style={{width: '100%'}}
          placeholder={l('pages.Scheduler.label.process.switchcluster')}
