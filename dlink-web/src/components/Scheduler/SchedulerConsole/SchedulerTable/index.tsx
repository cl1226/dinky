...
import {showJobData} from "@/components/Scheduler/SchedulerEvent/DQL";
import {isSql} from "@/components/Scheduler/conf";
    ...
    return (<>
      {(current.console.result.jobId && (current.console.result.jobId.indexOf('unknown') === -1)) ? (<>
        <Button type="primary" onClick={showDetail} icon={<SearchOutlined/>}>
          {l('pages.Scheduler.label.result.query.latest.data')}
        </Button> &nbsp;
