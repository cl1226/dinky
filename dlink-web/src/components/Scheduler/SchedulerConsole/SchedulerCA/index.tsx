...
import {getLineage, getStreamGraph} from "@/pages/Scheduler/service";
...
const SchedulerCA = (props: any) => {
  ...
  const handleLineage = () => {
    ...
    res.then((result) => {
      if (result.datas) {
        ...
        message.error(l('pages.Scheduler.label.lineage.query.error','',{msg : result.msg}));
      }
    })
  };
  ...
  const handleExportStreamGraphPlan = () => {
    ...
    res.then((result) => {
      Modal.info({
        title: current.task.alias + l('pages.Scheduler.label.lineage.streamplan'),
        width: 1000,
        content: (
  ...
  return (<>
    <Tabs defaultActiveKey="Lineage" size="small" tabPosition="top" style={{border: "1px solid #f0f0f0"}}
          tabBarExtraContent={
            <>
              <Tooltip title= {l('pages.Scheduler.label.lineage.recalculate')}>
                <Button
                  type="text"
                  icon={<SearchOutlined/>}
                  onClick={handleLineage}
                >
                  {l('pages.Scheduler.label.lineage.calculate')}
                </Button>
              </Tooltip>
              {(!current.task.dialect || current.task.dialect == DIALECT.FLINKSQL) ?
                <Tooltip title={l('pages.Scheduler.label.lineage.export')}>
                  <Button
                    type="text"
