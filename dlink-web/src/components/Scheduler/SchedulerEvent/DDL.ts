...
import {executeDDL, getMSCatalogs} from "@/pages/Scheduler/service";
import FlinkSQL from "./FlinkSQL";
import {MetaStoreCatalogType, SessionType, TaskType} from "@/pages/Scheduler/model";
...
export function createSession(session: SessionType, dispatch: any) {
  const res = addOrUpdateData("api/Scheduler/createSession", session)
  res.then((result) => {
...
export function listSession(dispatch: any) {
  const res = getData("api/Scheduler/listSession");
  res.then((result) => {
    dispatch && dispatch({
      type: "Scheduler/saveSession",
...
export function changeSession(session: SessionType, dispatch: any) {
  dispatch && dispatch({
    type: "Scheduler/refreshCurrentSession",
    payload: session,
  });
  setTimeout(function () {
...
export function quitSession(dispatch: any) {
  dispatch && dispatch({
    type: "Scheduler/quitCurrentSession",
  });
}
...
export function clearSession(session: string, dispatch: any) {
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      const res = handleRemove('/api/Scheduler/clearSession', [para]);
      res.then((result) => {
...
export function showSessionCluster(dispatch: any) {
  ...
  res.then((result) => {
    result.datas && dispatch && dispatch({
      type: "Scheduler/saveSessionCluster",
      payload: result.datas,
    });
  });
}
...
export function showFlinkJobs(clusterId: number) {
  return getData('api/Scheduler/listJobs', {clusterId: clusterId});
}

/*--- 停止 Flink Jobs ---*/
export function cancelJob(clusterId: number, jobId: string) {
  return getData('api/Scheduler/cancel', {clusterId: clusterId, jobId: jobId});
}
...
export function savepointJob(clusterId: number, jobId: string, savePointType: string, name: string, taskId: number) {
  return getData('api/Scheduler/savepoint', {clusterId, jobId, savePointType, name, taskId});
}
...
export function showClusterConfiguration(dispatch: any) {
  ...
  res.then((result) => {
    result.datas && dispatch && dispatch({
      type: "Scheduler/saveClusterConfiguration",
      payload: result.datas,
    });
  });
}
