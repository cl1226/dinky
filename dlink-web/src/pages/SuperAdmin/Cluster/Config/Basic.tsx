...
const BasicTab: React.FC<ITabComProps> = (props: ITabComProps) => {
  ...
  const onLoadConfig = async () => {
    ...
    const configRes = await getClusterConfig(params)
  ...
  const setBasicFormValue = (info: ICluster) => {
    const extraObj = {
      CDH: {
        url: info.url,
  ...
  const setKerberosFormValue = (info: ICluster) => {
  ...
  useEffect(() => {
    if ((mode === 'edit' || mode === 'view') && detailInfo && detailInfo.cluster) {
      setCollapseKey([...collapseKey, 'config', 'kerberos'])
      setConfigLoaded(true)
