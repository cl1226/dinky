...
import type { HadoopTableListItem } from '@/pages/RegistrationCenter/data'
...
import ClusterForm from '@/pages/RegistrationCenter/ClusterManage/Hadoop/components/ClusterForm'
...
const ClusterTableList: React.FC<{}> = (props: any) => {
  ...
  const [row, setRow] = useState<HadoopTableListItem>()
  const [selectedRowsState, setSelectedRows] = useState<HadoopTableListItem[]>([])

  const editAndDelete = (key: string | number, currentItem: HadoopTableListItem) => {
    if (key === 'edit') {
  ...
  const MoreBtn: React.FC<{
    item: HadoopTableListItem
  }> = ({ item }) => (
    <Dropdown
      overlay={
        <Menu onClick={({ key }) => editAndDelete(key, item)}>
          {item.autoRegisters == 1 ? (
  ...
  const columns: ProColumns<HadoopTableListItem>[] = [
    {
  ...
  return (
    <PageContainer title={false}>
      ...
      >
        {row?.name && (
          <ProDescriptions<HadoopTableListItem>
            column={2}
            title={row?.name}
            request={async () => ({
              data: row || {},
