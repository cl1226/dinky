...
import { saveSettings } from '@/pages/Resource/FlinkManage/FlinkSettings/function'
  ...
  const getData = () => [
    {
      ...
      actions:
        editName != 'sqlSubmitJarPath'
          ? [<a onClick={({}) => handleEditClick('sqlSubmitJarPath')}>{l('button.edit')}</a>]
          : [
    ...
    },
    {
      ...
      actions:
        editName != 'sqlSubmitJarParas'
          ? [<a onClick={({}) => handleEditClick('sqlSubmitJarParas')}>{l('button.edit')}</a>]
          : [
    ...
    },
    {
      ...
      actions:
        editName != 'sqlSubmitJarMainAppClass'
          ? [
              <a onClick={({}) => handleEditClick('sqlSubmitJarMainAppClass')}>
                {l('button.edit')}
              </a>,
            ]
          : [
              <a onClick={({}) => handleSaveClick('sqlSubmitJarMainAppClass')}>
                {l('button.save')}
              </a>,
              <a onClick={({}) => handleCancelClick()}>{l('button.cancel')}</a>,
            ],
...
export default connect(({ FlinkSettings }: { FlinkSettings: SettingsStateType }) => ({
  ...
  useRestAPI: FlinkSettings.useRestAPI,
  sqlSeparator: FlinkSettings.sqlSeparator,
  jobIdWait: FlinkSettings.jobIdWait,
}))(FlinkConfigView)
