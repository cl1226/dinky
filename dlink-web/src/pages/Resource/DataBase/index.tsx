...
import { getDBImage } from '@/pages/Resource/DataBase/DB'
import DBForm from './components/DBForm'
import { ActionType } from '@ant-design/pro-table'

import styles from './index.less'
import { DataBaseItem } from '@/pages/Resource/data'
import { checkHeartBeat, copyDatabase } from '@/pages/Resource/DataBase/service'
...
const DataBaseTableList: React.FC<{}> = (props: any) => {
  ...
  const onDeleteDataBase = (row: DataBaseItem) => {
    Modal.confirm({
      ...
      content: l('pages.rc.db.deleteConfirm', '', {
        dbName: row.alias === '' ? row.name : row.alias,
      }),
        ...
        renderItem={(row) => {
          return (
            ...
            >
              <Card.Meta
                title={
                  <Tag color="gray" key={row.alias}>
                    {row.alias}
                  </Tag>
                }
                description={
                  <Space size={0}>
                    <Tag key={row.name}>{row.name}</Tag>
